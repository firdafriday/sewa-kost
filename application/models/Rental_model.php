<?php
	...
	class Rental_model extends CI_Model{
		...
		public function ambil_id_kos($id){
			$hasil = $this->db->where('id_kos',$id)->get('kos');
			if($hasil->num_rows() > 0){
				return $hasil->result();
			}else{
				return false;
		...
		public function total_data_rental(){
			...
			$kos 				= $this->db->get_where('kos', array('nama_rental' => $nama_rental))->num_rows();
		...
		public function total_data_admin(){
			...
			$kos 				= $this->db->count_all_results('kos');
